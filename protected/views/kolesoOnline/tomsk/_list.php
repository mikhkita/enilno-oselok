        ...
        <li  <? unset($_GET['partial'],$_GET['GoodFilter_page']); echo "data-last='".$last."'" ?> class="gradient-grey b-good-type-<?=$good->good_type_id?>">
            ...
            <div class="params-cont">
                ...
                <? elseif($type == 1): ?>
                    ...
                    <? $available = Interpreter::generate($this->params[$type]["AVAILABLE"], $good,$dynamic); if(!$good->fields_assoc[27]->value) $available = ""; $available = ($available != "В наличии") ? "Доставка ".$available : $available; ?>
                    <h5><?=$available;?></h5>
                    <h6><?=Interpreter::generate($this->params[$type]["TITLE_2_CODE"], $good,$dynamic);?> <?=$params[$type]["CATEGORY"]["WEAR"]["LABEL"]?>: <?=$good->fields_assoc[$params[$type]["CATEGORY"]["WEAR"]['ID']]->value?> %</h6>
                <? elseif($type == 3): ?>
                ...
                <? if($price != Yii::app()->params["zeroPrice"] && !$isHidePrice): ?>
                    <? if(!$mobile): ?>
                        <? if(isset($_SESSION["BASKET"]) && array_search($good->id, $_SESSION["BASKET"]) !== false): ?>
                            <a href="<?=Yii::app()->createUrl('/kolesoOnline/basket',array('id' => $good->id,'add' => true))?>" class="b-black-butt carted">Оформить</a>
                        <? else: ?>
                            <a href="<?=Yii::app()->createUrl('/kolesoOnline/basket',array('id' => $good->id,'add' => true))?>" class="b-black-butt to-cart">В корзину</a>
                        <? endif; ?>
                    <? else: ?>
                        <a href="<?=Yii::app()->createUrl('/kolesoOnline/cart',array('id' => $good->id))?>" class="b-black-butt">Купить</a>
                    <? endif;?>
                <? else: ?>
                    <a href="#" class="fancy b-black-butt acc" data-block="#b-popup-buy" data-aftershow="category_buy">Уточнить цену</a>
                <? endif; ?>
