			...
			<div class="detail-wrap clearfix">
				...
				<div class="detail-price gradient-grey right">
					<? $price = Interpreter::generate($this->params[$_GET['type']]["PRICE_CODE"], $good, $dynamic);?>
						<? $price = number_format(intval($price), 0, ',', ' ' )." р."; $order = Interpreter::generate($this->params[$_GET['type']]["ORDER"], $good,$dynamic); ?>
						<? $price = ( !$good->fields_assoc[20]->value || $good->fields_assoc[20]->value == 0 || $good->good_type_id == 3 || $isHidePrice)? Yii::app()->params["zeroPrice"] : $price; ?>
				...
				<div class="detail-desc left">
					...
					<ul>
						...
						<? if( isset($attr["TYPE"]) && $attr["TYPE"] == "INTER" ): ?>
							<li><?=$attr['LABEL']?>: <span><?=((Interpreter::generate($attr['ID'], $good, $dynamic)) ? Interpreter::generate($attr['ID'], $good, $dynamic) : "Не указано"); if(Interpreter::generate($attr['ID'], $good, $dynamic) != "Неизвестен" && Interpreter::generate($attr['ID'], $good, $dynamic)) echo $attr['UNIT']?></span></li>
						<? else: ?>
						...
						<? if( $this->user && isset($good->fields_assoc[110]) && $good->fields_assoc[110]->value != ""): ?>
							<li>Место на складе: <span><?=$good->fields_assoc[110]->value?></span></li>
						<? endif; ?>
