...
<?php $form=$this->beginWidget('CActiveForm'); ?>
	<table class="b-table" border="1">
		<tr>
			...
			<th><? echo $labels['title']; ?></th>
			<th><? echo $labels['category_id']; ?></th>
			<th style="width: 150px;">Действия</th>
		</tr>
		<tr class="b-filter">
			<td></td>
			<td><?php echo CHtml::activeTextField($filter, 'title'); ?></td>
			<td><?php echo CHtml::activeDropDownList($filter, 'category_id', array(""=>"Все категории")+CHtml::listData(GoodType::model()->findAll(), 'id', 'name')); ?></td>
		...
		<? if( count($data) ): ?>
			<? foreach ($data as $i => $item): ?>
				<tr<?if(isset($_GET["id"]) && $item->id == $_GET["id"]):?> class="b-refresh"<?endif;?>>
					<td><?=$item->id?></td>
					<td class="align-left"><a href="<?php echo $this->createUrl('/export/admindynamic',array('id'=>$item->id))?>"><?=$item->title?></a></td>
					<td class="align-left"><?=$item->goodType->category_id?></td>
