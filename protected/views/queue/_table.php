...
<?php $form=$this->beginWidget('CActiveForm'); ?>
	<table class="b-table" border="1">
		<tr>
			...
			<th>Товар</th>
			<? if( $category->id != 3875 ): ?><th>Город</th><? endif; ?>
		...
		<? if( count($data) ): ?>
			<? foreach ($data as $i => $item): ?>
				<tr<?if(isset($_GET["id"]) && $item->id == $_GET["id"]):?> class="b-refresh"<?endif;?>>
					...
					<? if( $category->id != 3875 ): ?><td class="align-left"><b><?=$item->advert->city->value?></b><?if($category->id == 2047):?> <?=$item->advert->type->value?><?endif;?></td><? endif; ?>
