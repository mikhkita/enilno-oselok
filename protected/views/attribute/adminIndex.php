...
<?php $form=$this->beginWidget('CActiveForm'); ?>
	<table class="b-table" border="1" data-warning="Все связанные с атрибутом справочники будут удалены!">
		...
		<? if( count($data) ): ?>
			<? $cities = array(); foreach ($data as $i => $item): ?>
				<? if($item->folder == 1): array_push($cities, $item); else: ?>
				<tr<?if(isset($_GET["id"]) && $item->id == $_GET["id"]):?> class="b-refresh"<?endif;?>>
					...
					<td class="align-center"><?=(($item->required)?("<span class='b-circle'></span>"):("-"))?></td>
