<div style="display:none;">
	...
	<?=CHTML::beginForm(Yii::app()->createUrl('/good/adminindex',array('goodTypeId' => $_GET['goodTypeId'],'GoodFilter_page' => 1)),'POST',array('id'=>'b-filter-form'))?>
		...
		<div class="b-filter-block">
			...
			<div class="clearfix">
				<? if( $item["VIEW"] == "CHECKBOX" ): ?>	
					<?=CHTML::checkBoxList($arr_name."[$field]", $filter_values[$field], $item["VARIANTS"], array("separator"=>"","template"=>"<div class='b-filter-checkbox'>{input}{label}</div>")); ?>
		...
		<?=CHTML::dropDownList("sort[type]", $_POST["order"]["type"], array("ASC"=>"По возрастанию", "DESC"=>"По убыванию"),array('id'=>'b-order-2','class'=>'hidden')); ?>
