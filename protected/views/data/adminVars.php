			...
			<? foreach ($data as $i => $item): ?>
				<tr<?if(isset($_GET["id"]) && $item->name == $_GET["id"]):?> class="b-refresh"<?endif;?>>
					...
					<td class="b-tool-cont">
						<a href="<?php echo Yii::app()->createUrl('/'.$this->adminMenu["cur"]->code.'/adminvarsupdate',array('id'=>$item->name))?>" class="ajax-form ajax-update b-tool b-tool-update" title="Редактировать"></a>
						<a href="<?php echo Yii::app()->createUrl('/'.$this->adminMenu["cur"]->code.'/adminvarsdelete',array('id'=>$item->name))?>" class="ajax-form ajax-delete b-tool b-tool-delete" data-warning="Вы действительно хотите удалить переменную &quot;<?=$item->name?>&quot;?" title="Удалить"></a>
					</td>
				</tr>
			<? endforeach; ?>
		<? else: ?>
